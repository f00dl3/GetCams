...
(
curl --max-time 3 "http://IPCam1:88/cgi-bin/CGIProxy.fcgi?cmd=snapPicture2&usr=$ipCamUser&pwd=$ipCamPass" > /dev/shm/GetCams/Xwebc3-temp.jpeg &
curl --max-time 3 "http://IPCam2:88/cgi-bin/CGIProxy.fcgi?cmd=snapPicture2&usr=$ipCamUser&pwd=$ipCamPass" > /dev/shm/GetCams/Xwebc2-temp.jpeg
)

if [ ! -s /dev/shm/GetCams/Xwebc3-temp.jpeg ]; then
	curl --max-time 3 "http://IPCam1WiFi:88/cgi-bin/CGIProxy.fcgi?cmd=snapPicture2&usr=$ipCamUser&pwd=$ipCamPass" > /dev/shm/GetCams/Xwebc3-temp.jpeg
fi
...
if [ ! -f /dev/shm/GetCams/Xwebc1-temp.jpeg ]; then
	...
	if [ $UACam1 -eq $mTrigger ]; then
		echo "Camera 1 offline more than $mTrigger cycles!" | mail -s "Camera 1 offline!" $SendTo
	fi
...
if [ ! -s /dev/shm/GetCams/Xwebc2-temp.jpeg ]; then
	...
	if [ $UACam3 -eq $mTrigger ]; then
		echo "Camera 2 offline more than $mTrigger cycles!" | mail -s "Camera 2 offline!" $SendTo
	fi
...
if [ ! -s /dev/shm/GetCams/Xwebc3-temp.jpeg ]; then
	...
	UACam3T=$((UACam3T + 1))
	if [ $UACam3 -eq $mTrigger ]; then
		echo "Camera 3 offline more than $mTrigger cycles!" | mail -s "Camera 3 offline!" $SendTo
	fi
